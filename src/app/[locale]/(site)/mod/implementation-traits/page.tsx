...
export default async function ModImplementationTraitsPage() {
  ...
  if (!ok) {
    return (
      <main className="mx-auto w-full max-w-4xl px-3 pb-[calc(6rem+env(safe-area-inset-bottom))] pt-6 min-[360px]:px-4">
        <h1 className="text-xl font-semibold font-kalam">工程特征管理</h1>
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-3 pb-[calc(6rem+env(safe-area-inset-bottom))] pt-6 min-[360px]:px-4">
      <div className="flex items-center justify-between gap-3">
        <h1 className="min-w-0 truncate text-xl font-semibold font-kalam">工程特征管理</h1>
        <Link
          href="/mod"
      ...
      <p className="mt-2 text-sm text-ink-light">
        共 {specs.length} 个工程特征。点击编辑可修改文案内容；图片请直接放入{" "}
        <code className="font-mono">public/implementation-traits/&lt;key&gt;/</code>。
      </p>
        ...
        {specs.map((spec) => (
          ...
          >
            ...
            <div className="flex shrink-0 items-center gap-2">
              <Link
                href={`/implementation-traits/${encodeURIComponent(spec.key)}`}
                target="_blank"
                className="sketch-button sketch-button-secondary text-sm"
              >
                预览
              ...
              <Link
                href={`/mod/implementation-traits/${encodeURIComponent(spec.key)}/edit`}
