...
export async function POST(req: NextRequest, { params }: RouteContext) {
  ...
  if (!isCorePatternKey(key)) {
    return NextResponse.json({ error: "Invalid pattern key" }, { status: 400 });
  }

  const existing = await readPatternSpec(key);
  if (!existing) {
    return NextResponse.json({ error: "Pattern meta not found" }, { status: 404 });
  }
  ...
  const dir = path.join(process.cwd(), "content", "patterns", key);
