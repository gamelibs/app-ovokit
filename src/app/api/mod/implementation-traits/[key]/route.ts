...
export async function POST(req: NextRequest, { params }: RouteContext) {
  ...
  if (!isImplementationTraitKey(key)) {
    return NextResponse.json({ error: "Invalid implementation trait key" }, { status: 400 });
  }

  const existing = await readImplementationTraitSpec(key);
  if (!existing) {
    return NextResponse.json({ error: "Implementation trait meta not found" }, { status: 404 });
  }
  ...
  const parsed = traitSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const dir = path.join(process.cwd(), "content", "implementation-traits", key);
