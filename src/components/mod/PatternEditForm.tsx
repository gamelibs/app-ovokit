...
export function PatternEditForm({ spec }: { spec: CorePatternSpec }) {
  const router = useRouter();
  const [form, setForm] = useState<CorePatternSpec>({ ...spec });
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [ok, setOk] = useState(false);

  function update<K extends keyof CorePatternSpec>(key: K, value: CorePatternSpec[K]) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setOk(false);
    try {
      const res = await fetch(`/api/mod/patterns/${encodeURIComponent(spec.key)}`, {
        method: "POST",
    ...
    <form onSubmit={submit} className="space-y-4">
      <div className="rounded-2xl sketch-border bg-paper p-4">
        <div className="grid gap-3">
          ...
          <label className="grid gap-1">
            <span className="text-xs font-semibold text-ink-muted font-kalam">核心循环</span>
            <input
              value={form.loop}
          ...
          <TagInput
            ...
            onChange={(v) => update("abstractions", v)}
          />
