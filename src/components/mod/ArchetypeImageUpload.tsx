...
export function ArchetypeImageUpload({
  archetypeKey,
  initialImages,
}: {
  archetypeKey: string;
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      const res = await fetch(`/api/mod/archetypes/${encodeURIComponent(archetypeKey)}/images`, {
        method: "POST",
        body: form,
      });
  ...
  async function remove(filename: string) {
    ...
    try {
      const res = await fetch(
        `/api/mod/archetypes/${encodeURIComponent(archetypeKey)}/images?filename=${encodeURIComponent(
          filename
        )}`,
        { method: "DELETE" }
      );
  ...
  return (
    <div className="space-y-3">
      ...
      {images.length === 0 ? (
        <p className="text-sm text-ink-light">暂无图片。上传后会保存在 public/archetypes/{archetypeKey}/</p>
      ) : (
          ...
          {images.map((name) => (
            <div key={name} className="group relative rounded-xl sketch-border bg-paper p-2">
              <div className="relative aspect-square w-full overflow-hidden rounded-lg">
                <Image
                  src={`/archetypes/${encodeURIComponent(archetypeKey)}/${encodeURIComponent(name)}`}
                  alt={name}
                  fill
